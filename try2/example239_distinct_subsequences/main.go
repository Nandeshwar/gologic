...
func main() {
	...
	*/
	...
	fmt.Println(distinctSubsequence2(s1, s2, 0, 0, dp))
}
...
func distinctSubsequence2(s1, s2 string, i, j int, dp [][]int) int {
	if j >= len(s2) {
		return 1 // j < 0 that means found
	}
	...
	if s1[i] == s2[j] {
		...
		cnt := distinctSubsequence2(s1, s2, i+1, j+1, dp) + distinctSubsequence2(s1, s2, i+1, j, dp) // i+1, j : fix substring and check other character in 1st string
		dp[i][j] = cnt
		return cnt
	}

	cnt := distinctSubsequence2(s1, s2, i+1, j, dp) // i+1, j : fix with s2 and try to match with all characters of s1
	dp[i][j] = cnt
	return cnt
}
